...
class AmbientAir:
    ...
    def __init__(self, pressure=1e5, temperature=20.):
...
class Road:
    ...
    def load_road_data(self, file_name, delimiter=','):
        ...
        # File opening
        try:
            # Attempt to open the data file. First line is avoided
    ...
    def __where(self, distance):
        """ Function that gives the index 'i' within the 'distance_from_start'
        list, for which 'distance_from_start[i] < distance'.
        """
        if (distance < 0) or (distance > self.road_length()):
            raise ValueError("Argument must be within the road range distance!")
        return int((~ (self.__distance_from_start > distance) * 1).sum() - 1)
    ...
    def slope(self, distance):
        ...
        slope = (self.__elevation_from_start[i + 1] - self.__elevation_from_start[i]) / \
    ...
    def climbing_angle(self, distance):
        ...
        from the starting point.
        """
        return float(np.rad2deg(np.asin(self.slope(distance))))
...
class LinearContinuousGearbox:
    ...
    def __get_gear_ratio(self, **kwargs):
        ...
        allowed_args = {'inlet_speed': float,
                        'outlet_speed': float,
                        'inlet_frequency': float,
                        'outlet_frequency': float}
        ...
        for key, value in kwargs.items():
            ...
            if not isinstance(value, allowed_args[key]):
                raise TypeError(f"Argument '{key}' must be of type\
        ...
        if 'outlet_speed' in kwargs:
            variable = kwargs['outlet_speed'] / self.maximum_outlet_speed()
        if 'outlet_frequency' in kwargs:
            variable = kwargs['outlet_frequency'] / \
                self.maximum_outlet_frequency()
        gear_ratio = (self.maximum_gear_ratio - self.minimum_gear_ratio) * \
    ...
    def set_inlet_torque(self, torque):
        ...
        # Outlet
        self._outlet_torque = self.energy_efficiency * \
            torque / self.gear_ratio()
...
class SynchronousElectricMotor:
    ...
    def set_base_speed(self, speed):
        ...
        self._base_frequency = speed * 30. / np.pi
    ...
    def set_maximum_speed(self, speed):
        ...
        self._maximum_frequency = speed * 30. / np.pi
    ...
    def set_maximum_frequency(self, freq):
        ...
        self._maximum_speed = freq * np.pi / 30.

    maximum_frequency = property(fget=get_maximum_frequency,
                                 fset=set_maximum_frequency)
    ...
    maximum_power = property(fget=get_maximum_power,
                             fset=set_maximum_power)
...
class Vehicle:
    ...
    def acceleration(self, motive_force, slope, speed, **kwargs):
        ...
        acceleration.
        """
        return (motive_force - self.resistance_force(slope, speed, **kwargs)) /\
            self.inertial_mass()
    ...
    def drag_resistance(self, speed, **kwargs):
        ...
        if 'air_density' in kwargs:
            ...
            rho = 1.2
        return self.frontal_area * self.drag_coefficient *\
            0.5 * rho * pow(speed, 2)
    ...
    def motive_force(self, acceleration, slope, speed, **kwargs):
        ...
        """
        return self.inertial_mass() * acceleration +\
            self.resistance_force(slope, speed, **kwargs)
    ...
    def resistance_force(self, slope, speed, **kwargs):
        ...
        """
        return self.climbing_resistance(slope) + \
...
class VehicleDynamicsModel:
    ...
    ----------
    time_step : float
        Time step used in the calculation, in s.
    average_acceleration : float
        Average value of the vehicle positive acceleration used as a setpoint
        when a speed increase is required, in m/s².
    average_deceleration : float
        Average value of the vehicle negative acceleration when a speed
        decrease is required, in m/s².

    Methods
    -------
    run_calculation()
        Run the vehicle dynamics model.
    """
    ...
    def run_calculation(self, initial_speed=0., **kwargs):
        ...
        here in km/h.
        """
        time, distance, speed, acceleration = 0., 0., initial_speed / 3.6, 0.
        self.results['time'] = [time]
        self.results['distance'] = [distance]
        self.results['speed'] = [speed]
        self.results['acceleration'] = [acceleration]
        self.results['power'] = [0.]
        self.results['motor_torque'] = [0.]
        self.results['motor_speed'] = [0.]
        ...
        while distance < self.road.road_length():
            ...
            if speed < speed_limit:
                ...
                # If not
                    ...
                    force_wheels = self.gearbox.outlet_torque / \
                        self.vehicle.wheels_radius
                    # And then the actual acceleration reachable
                    acceleration = self.vehicle.acceleration(force_wheels,
                                                             slope,
                                                             speed, **kwargs)
            ...
            # Otherwise, the current speed is equal to the limit one
                ...
                self.gearbox.outlet_torque = self.vehicle.resistance_force(
                    slope, speed) * self.vehicle.wheels_radius
                # The resulting gearbox inlet torque must be compared with the
            ...
            # Speed being now known, distance and time can be calculated
            distance += speed * self.time_step
            time += self.time_step
            # And results can now be stored in the dedicated dictionary
            if distance <= self.road.road_length():
    ...
    def speeds(self):
        ...
        if not self.results:
            return {}
        return {'min': min(self.results['speed']),
